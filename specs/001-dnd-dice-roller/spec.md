...
### User Story 2 - Roll Dice Using Dice Notation (Priority: P2)

A player selects "Enter Notation" from the main menu and types a dice expression such as `3d8+2`, `1d20+5`, `2d6-1`, or `d20`. The application parses the notation, validates it, rolls the dice, and displays the results. If the notation is invalid, a clear error message is shown and the user can try again.

**Why this priority**: Dice notation is the natural language of tabletop gaming. Enabling fast expression entry makes the app useful for experienced players without extra prompts, but it depends on the roll display logic established in P1.

**Independent Test**: Can be fully tested by launching the app, selecting "Enter Notation", submitting a valid expression, and verifying results — and separately submitting an invalid string to confirm an error appears.
...
5. **Given** a valid dice notation is entered, **When** the roll expression is already in recent history, **Then** the expression moves to the top of the history rather than creating a duplicate.

---

### User Story 3 - Reroll From History (Priority: P3)

After rolling several different expressions, a player selects "Reroll Recent" from the main menu. The application displays the last up to 7 unique roll expressions. The player selects one and the application immediately re-executes that roll, displaying new results. The selected expression moves to the top of the history list.
...
3. **Given** the user completes a roll or encounters an error, **When** the result or error is shown, **Then** the application returns to the main menu automatically.
...
- **FR-002**: The application MUST allow the user to specify the number of dice to roll (minimum 1).
- **FR-003**: The application MUST allow the user to specify a positive or negative integer modifier (including zero / no modifier).
...
- **FR-006**: The application MUST accept standard dice notation strings in the format `XdY`, `XdY+Z`, `XdY-Z`, and `dY` (which MUST be treated as `1dY`).
- **FR-007**: The application MUST validate all dice notation input and display a clear, user-friendly error message for any unparseable or out-of-range expression; it MUST NOT crash on invalid input.
- **FR-008**: The application MUST maintain a roll history of up to 7 unique normalized roll expressions per session.
- **FR-009**: Rolling the same normalized expression as one already in history MUST move that entry to the top of the history without creating a duplicate.
...
- **FR-012**: The main menu MUST provide at least four options: guided new roll, enter notation, reroll recent, and exit.
...
- **DiceExpression**: Represents a parsed roll specification — number of dice, dice type (sides), and integer modifier. This is the normalized form stored in history.
...
- **SC-003**: Dice notation parsing and result display complete instantaneously (imperceptible delay) for any valid expression.
- **SC-004**: Roll history never contains more than 7 entries and never contains duplicate normalized expressions.
- **SC-005**: A repeated roll expression is always promoted to position 1 in history without increasing the history count.
- **SC-006**: Every invalid notation input is handled gracefully with a descriptive error message; the application never terminates unexpectedly due to bad input.
